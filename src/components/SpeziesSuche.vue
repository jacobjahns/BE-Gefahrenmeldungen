<template>
  <div class="speziesSuche">
    <div class="column-head">
      <input type="input" class="speziesSuche-suche" placeholder="Neue Spezies" @focus="requestSpecies" v-model="searchInput" @input="searchSpecies">
    </div>
    <div class="column-body speziesSuche-ergebnisse">
      <div @click="addQueries">ADD</div>
      <div class="speziesSuche-ergebnis" v-for="s in this.species" :key="s.id">
        <router-link to="/">{{s.id}}</router-link>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    requestSpecies() {
      ...
      if(now - this.lastRequest > 60000) {
        ...
        // fetch("https://api.beachexplorer.org/v1/species")
        //   .then(res => res.json)
        //   .then(data => {
    ...
    addQueries() {
      let p = ['Walgesichtsfischmenschkopf', 'Haifisch'];
        this.$router.replace({query: {species: p}});
    }
  }
}
</script>
